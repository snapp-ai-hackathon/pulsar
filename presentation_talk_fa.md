...
3. **زیرساخت MLOps هوشمند** شامل MLflow و NATS که این مدل‌ها را به یک سیستم قابل استقرار روی ناوگان واقعی تبدیل می‌کند و با استک فعلی اسنپ (ClickHouse، Kubernetes، Docker) هماهنگ است.

بقیه‌ی لایه‌ها (ClickHouse، فایل‌های CSV، Docker، Helm، GitHub Actions) فقط این مغز ML را به یک محصول صنعتی و قابل استقرار سراسری تبدیل می‌کنند.  
...
3. **چرا الان قابل اجراست؟**  
   - داده‌ی واقعی از `ClickHouse` و CSV داریم و یک دیتاست زمان‑سری ساخته‌شده در `cache/timeseries` الان روی ریپو موجود است.  
   - لایه‌ی `TimeSeriesStore` و مدل‌های ML (`ElasticNet`, `TemporalCNN`) پیاده‌سازی و روی همین دیتاست **ترین شده‌اند**؛ آرتیفکت‌های آموزش در `mlruns/` داخل ریپو ثبت شده‌اند.  
   - با `NATS` می‌توانیم *آموزش مدل* را رویدادمحور کنیم و در آینده با زیرساخت اصلی اسنپ یکپارچه شویم (الگوی فعلی روی NATS لوکال قابل‌اجراست).
...
MLflow ستون فقرات **رصد و مدیریت مدل‌ها** در Pulsar است و در نسخه‌ی فعلی، آرتیفکت‌های چند ران واقعی آن داخل پوشه‌ی `mlruns/` همین ریپو موجود است:
...
> «ما فقط مدل نساختیم، بلکه چرخه‌ی کامل *Data → Model → Metrics → Registry* را هم در نظر گرفتیم، مطابق best practiceهای MLOps – و آرتیفکت همین چرخه (mlruns + cache/timeseries) الان داخل ریپو و ایمیج داکر موجود است.»
