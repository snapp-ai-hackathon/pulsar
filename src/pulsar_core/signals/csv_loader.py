...
class CSVSignalLoader:
    ...
    def acceptance_metrics(self, period: PeriodWindow, hexagon: int, service_type: int) -> Dict[str, int]:
        ...
        requests = int(surge_values.sum() * 10 + len(period_data) * 5)  # Scale based on surge
    ...
    def price_metrics(
        ...
        return {"ride_requests": max(ride_requests, 0), "get_prices": max(get_prices, 0)}
    ...
    def surge_metrics(
        ...
        return {
            ...
            "cumulative_surge_percent": float(latest_row.get("cumulative_surge_percent", 0.0)),
            "cumulative_surge_absolute": float(latest_row.get("cumulative_surge_absolute", 0.0)),
        }

    def _filter_period(self, period: PeriodWindow, hexagon: int, service_type: int) -> pd.DataFrame:
        ...
        if self.csv_data['clickhouse_time'].dtype != 'datetime64[ns]':
            self.csv_data['clickhouse_time'] = pd.to_datetime(self.csv_data['clickhouse_time'])
...
def create_import_tasks_from_csv(
    ...
    csv_data['period_start'] = csv_data['clickhouse_time'].apply(
        lambda ts: with_duration(ts, period_duration_minutes, collect_duration_minutes).start
    )
